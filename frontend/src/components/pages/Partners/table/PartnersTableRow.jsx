...
export function PartnersTableRow({ partner, getData }) {
  ...
  async function deletePartner() {
    const confirmed = window.confirm(
      `Are you sure you want to delete the partner ${partner.clientName || partner.clientFullName}?`
    );
  ...
  return (
    <tr>
      ...
      <td>{partner.partnerEgn || "N/A"}</td>
      <td>{partner.partnerBulstat || "N/A"}</td>
      <td>{partner.clientType === "individual" ? "Individual" : "Business"}</td>
      ...
      <td>
        <button
          ...
          onClick={() => navigate(`/partners/edit/${partner.partnerId}`)}
        >
          Edit
        </button>
